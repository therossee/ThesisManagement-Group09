...
function MobStartRequestsList({ data, isLoading, buttonsLoading, acceptStartRequest, rejectStartRequest, showModal }) {
    ...
    return (
            <div style={{ marginRight: "5%", marginLeft: "5%", marginBottom: '70px' }}>
                <Collapse accordion>
                    {data.map((request) => (
                        <Collapse.Panel
                            key={request.id}
                            title={<>
                            <div>
                                <Badge status="default">{request.student.surname + " " + request.student.name + " (" + request.student.id + ") "}</Badge>
                            </div>
                                <span> </span>
                                {request.title}
                                 </>}>
                            <div>
                                    <h4>Status:</h4>
                                <div><span style={{ color: `${getStatusColor(request.status)}` }}>{request.status}</span></div>
                            </div>
                            <div >
